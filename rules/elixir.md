...
- Test with ExUnit (`async: true` when safe) plus property-based tests (StreamData); instrument with Telemetry. New behavior and bug fixes come test-first — a failing test before the code that satisfies it. The `tdd` skill covers that cycle.

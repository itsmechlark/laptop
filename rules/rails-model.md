...
- Never bypass validations — no `save(validate: false)`, `update_attribute`, or `toggle`. Validate the associated object (`user`), not its foreign-key column (`user_id`).
...
- Prevent N+1 with `includes`/`preload`/`eager_load` (catch regressions with the `bullet` gem); batch large reads with `find_each`/`in_batches`; wrap multi-step writes in a transaction.

...
`AGENTS.md` and `CLAUDE.md` load into every session, and each client caps how much of one it accepts. Two of those caps fail silently — content past the budget is dropped with nothing in the transcript to say so — so treat the budget as a constraint on the file rather than as advice.
...
- **Truncation takes the tail.** Codex cuts at the byte, so the last sections disappear first — usually the testing, verification, and commit conventions that conventionally sit at the bottom. If a file is near its budget, don't assume anything near the end was read.
...
- **Re-measure after any edit that grows one of these files** — `wc -c ~/.agents/AGENTS.md AGENTS.md` against 32768, plus each workspace file in turn. A file that crossed its budget reads exactly like a file that didn't, in every client.

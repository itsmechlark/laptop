...
| `lore` | A wrong trigger writes a file into a shared repository, which is the expense. Standalone by necessity rather than preference: its two nearest neighbors — `claude-handoff` and `standup` — both carry `disable-model-invocation`, so a shared pool with either would measure a skill the runner can never see fire. The measurable contest is with `pull-request`, `domain-modeling` and `draft-plan`, whose artifacts a note must not duplicate, so those supply the negatives here. The reverse direction is deliberately one-sided for now: no `lore` positive is planted in `domain-modeling.json`, because that set is a shared pool with `codebase-design` and a third member would change what the pair asserts |

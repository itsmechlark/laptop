...
This step files nothing and specs nothing. Once the list is agreed, `file-tickets` puts it on the tracker as one item per slice with the dependency edges intact, and `triage` works those items afterwards — creating tracker items is an outward-facing write that needs the user's yes, and both of those are user-invoked, so read and follow the target's `SKILL.md`. `draft-spec` writes slice one up properly when a contractor or an unattended agent has to work it cold.

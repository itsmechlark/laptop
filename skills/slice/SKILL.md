...
This step files nothing and specs nothing. `triage` owns the tracker, and creating items there is an outward-facing write that needs the user's yes (read and follow its `SKILL.md`); `draft-spec` writes slice one up properly when a contractor or an unattended agent has to work it cold.

...
- **Establish a green baseline before the first red.** Run the suite once before writing anything. On an already-failing suite you cannot tell your red from the pre-existing one, so "watch it fail" degrades into "watch something fail" — and the run that should have caught your mistake reads as noise you've learned to skip past. If the suite starts red, fix or quarantine the failure, or write down exactly which failures pre-date your work.

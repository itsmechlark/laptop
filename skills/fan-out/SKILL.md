...
- Not for writing the task briefs themselves — `draft-plan` turns one agreed slice into tasks with the interfaces each one consumes and produces, which is what makes them safe to run concurrently. This skill partitions and dispatches; it does not author what it sends
- Not for handing the current conversation off to a single background agent to finish — that's `agent-handoff`; fan-out is for splitting *new* work across several

...
**A global ADR stays out of the repo entirely.** It is your own record until the team adopts `docs/adr/`, so it earns no pointer in `AGENTS.md` and no inline citation anywhere in the tree — a repo reference to a file under `~/.agents/adr/` is a link the team cannot follow. The ADR becomes mentionable in the repo only once it moves into `docs/adr/` (drop the prefix, drop `metadata.repo`).

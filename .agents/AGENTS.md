...
- **Maintainability**
  ...
  - **Restrict comments in production code.** Prefer self-documenting code (precise names, small functions) over prose. A comment explains *why* — intent, a trade-off, a non-obvious constraint — never *what*, save where the mechanics are genuinely dense, like a non-obvious algorithm; if you're narrating what the code does, rename or refactor instead. Default to no comments, never leave commented-out code, and keep changelog or decision-log narration out of the source — that belongs in an ADR or the commit message. Tooling directives (`eslint-disable-*`, `@ts-expect-error`, `# rubocop:disable`) and a language's documentation layer (Elixir `@moduledoc`/`@doc`, published-API doc comments) are the standing exceptions. The `comments.md` rule carries this in full for Ruby, JavaScript, TypeScript, and Elixir.
...
- **Close out on the final tree.** Once the diff is stable, run `git diff --check` for whitespace damage and a diff-scoped secret scan (`gitleaks stdin --redact`) before handing off. When the change touched dependencies, lockfiles, or container images, also run a vulnerability scan (`trivy fs --scanners vuln .`) and treat new HIGH/CRITICAL findings as a failing gate. If the scan fails (missing DB, network unavailable, sandbox blocks the download), skip it and tell the user which command to run outside the agent session — don't silently omit the gate or claim it passed.

...
Git worktrees must live in the repo's dedicated sibling folder: `<repo-path>.worktrees/<worktree-name>` — e.g. `~/Codespace/monolith.worktrees/HDR-1234-feat-slug` for Monolith, `~/Codespace/hydra.worktrees/...` for Hydra. Name the worktree after its branch (see the `git-commit` skill for branch naming).

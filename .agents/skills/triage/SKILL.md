...
Step 4 runs on two other skills: `grilling` (one question at a time until the shape is agreed) and `domain-modeling` (sharpen the terms, then capture them in `CONTEXT.md` or an ADR).
